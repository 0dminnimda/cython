...
# @generated by pegen from Cython/Pegen/cython.gram
...
def parse_string(
    ...
    parser = CythonParser(tokenizer, verbose=verbose, py_version=py_version)
...
class CythonParser(Parser):
    ...
    def func_type_comment(self) -> Optional[Any]:
        ...
        if (
            ...
            (t := self.type_comment())
            and
            self.positive_lookahead(self._tmp_141, )
        ):
            return t . string
        self._reset(mark)
        if (
    ...
    def invalid_default(self) -> Optional[Any]:
        ...
        if (
            (a := self.expect('='))
            and
            self.positive_lookahead(self._tmp_167, )
        ):
    ...
    def invalid_with_item(self) -> Optional[NoReturn]:
        ...
        if (
            ...
            (a := self.expression())
            and
            self.positive_lookahead(self._tmp_191, )
        ):
            return self . raise_syntax_error_invalid_target ( Target . STAR_TARGETS , a )
        self._reset(mark)
        return None
    ...
    def invalid_try_stmt(self) -> Optional[NoReturn]:
        ...
        if (
            ...
            (block := self.block())
            and
            self.negative_lookahead(self._tmp_200, )
        ):
    ...
    def invalid_double_starred_kvpairs(self) -> Optional[None]:
        ...
        if (
            ...
            (a := self.expect(':'))
            and
            self.positive_lookahead(self._tmp_209, )
        ):
    ...
    KEYWORDS = ('from', 'as', 'and', 'in', 'None', 'not', 'nonlocal', 'assert', 'class', 'else', 'elif', 'global', 'async', 'import', 'False', 'try', 'if', 'with', 'raise', 'await', 'return', 'yield', 'True', 'finally', 'or', 'except', 'while', 'del', 'for', 'def', 'is', 'continue', 'lambda', 'pass', 'break')
