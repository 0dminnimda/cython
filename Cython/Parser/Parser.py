...
import sys
import token
from typing import Any, List, NoReturn, Tuple, TypeVar, Union

from ..Compiler import Builtin, ExprNodes, Nodes, Parsing
...
class Parser(Parser):
    ...
    def make_string(self, tokens, pos):
        ...
        scanner = PyrexScanner(
            io.StringIO(source), self.source_desc, parent_scanner=self.streamer,
            source_encoding=self.streamer.source_encoding, initial_pos=pos)
...
class CythonParser(Parser):
    ...
    def literal_expr(self) -> Optional[Any]:
        ...
        if __true_result:
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ExprNodes . BoolNode ( self . pos ( lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset ) , value = True );
        self._reset(mark)
        __true_result = False
        while 1:  # for early false result as in the 'A and B'
            __last = literal = self.expect('False')
        ...
        if __true_result:
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ExprNodes . BoolNode ( self . pos ( lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset ) , value = False );
        self._reset(mark)
        return None;
    ...
    def atom(self) -> Optional[Any]:
        ...
        if __true_result:
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ExprNodes . EllipsisNode ( self . pos ( lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset ) );
        self._reset(mark)
        return None;
